...
class TestGetOrderUser:
    ...
    def test_get_order_user_with_auth(self, create_user):
        ...
        requests_create_order = requests.post(f"{Urls.MAIN_URL}{Urls.MAKE_ORDER}", headers=token, data=Ingredient.valid_ingredients_data)
        response_get_order = requests.get(f"{Urls.MAIN_URL}{Urls.GET_ORDERS}", headers=token)
    ...
    def test_get_order_user_not_auth(self):
        r = requests.get(f"{Urls.MAIN_URL}{Urls.GET_ORDERS}")
